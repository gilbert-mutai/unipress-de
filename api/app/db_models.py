...
class Claim(Base):
    ...
    key: Mapped[str] = mapped_column(String(16))  # e.g. "clm_001"
    text: Mapped[str] = mapped_column(Text)
    claim_type: Mapped[str] = mapped_column(String(20))
    # Flattened SourceSpan (provenance, quote-verified).
    page: Mapped[int] = mapped_column(Integer)
    section: Mapped[str | None] = mapped_column(String(200), nullable=True)
    char_start: Mapped[int] = mapped_column(Integer)
    char_end: Mapped[int] = mapped_column(Integer)
    ...
    numeric: Mapped[bool] = mapped_column(default=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
...
class SentenceRecord(Base):
    ...
    timecode: Mapped[str | None] = mapped_column(String(40), nullable=True)
    on_screen: Mapped[str | None] = mapped_column(Text, nullable=True)
