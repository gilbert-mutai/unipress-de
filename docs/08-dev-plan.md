...
**Status — 🚧 in progress. Workstreams A (eval harness) + C (MLflow) + D (CI eval-gate) + E (Grafana/observability) ✅ delivered (21 Jul 2026); only B (human-verified gold set) remains.**
...
- **Remaining in P5:** **B** — human-verified gold + adversarial set (also closes the P1 tail). Bootstrap candidates from the extractor, human-verify, drop `eval/gold/<paper_id>.yaml`, and the gold coverage + adversarial-caught metrics (and their dashboard series) activate automatically.
