...
**Status — 🚧 in progress. Workstreams A (eval harness) + D (CI eval-gate) ✅ delivered (21 Jul 2026).**
...
- **D — Built:** the **CI eval-gate** — a new `eval-gate` job in `.github/workflows/ci.yml` runs the metric/gate unit tests and executes the harness on an **in-memory synthetic fixture paper** (`run_eval.py --synthetic`, since the real PDFs are gitignored) with `--fail-on-target-miss`, failing the build if a headline metric regresses past the §6 bars. **Verified:** the gate passes clean metrics and a dedicated test proves it **fails on an injected regression** (hallucination spike + faithfulness drop) — 13 eval tests green; `uv sync --frozen --group eval` resolves.
- **Remaining in P5:** **B** — human-verified gold + adversarial set (also closes the P1 tail); **C** — MLflow tracking + retrieval A/B; **E** — Grafana board (per-stage latency, token cost, cache-hit, queue depth + live eval series).
